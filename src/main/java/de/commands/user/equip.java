        ...
        if (arg1.equals("weapon")) {
            if (arg2.matches("[0-9]+")) {
                if (Database.equipItemInInventory(event.getMember().getId(), Integer.parseInt(arg2), "e_weapon", "weapon")) {
                    currchat.sendMessage(createEmbed.methode("**Success**", "Succesfully changed your equipped weapon!", Color.green, null, null, null).build()).complete();
                } else {
                    //Had problem finding ID
                    currchat.sendMessage(createEmbed.methode("**ERROR**", "Could not find the Weapon you were looking for.", Color.red, null, null, null).build()).complete();
                }
            } else {
            ...
            if (arg2.matches("[0-9]+")) {
                if (Database.equipItemInInventory(event.getMember().getId(), Integer.parseInt(arg2), "e_armor", "armor")) {
                    currchat.sendMessage(createEmbed.methode("**Success**", "Succesfully changed your equipped armor!", Color.green, null, null, null).build()).complete();
                } else {
                    //Had problem finding ID
                    currchat.sendMessage(createEmbed.methode("**ERROR**", "Could not find the Armor you were looking for.", Color.red, null, null, null).build()).complete();
                }
            } else {
                //Error no correct number
                currchat.sendMessage(createEmbed.methode("**ERROR**", "Wrong Syntax! The **item ID** you typed is **wrong**, check the item ID in your inventory!", Color.red, null, null, null).build()).complete();
            ...
            if (arg2.matches("[0-9]+")) {
                if (Database.equipTitleInInventory(event.getMember().getId(), Integer.parseInt(arg2), "e_title")) {
                    currchat.sendMessage(createEmbed.methode("**Success**", "Succesfully changed your equipped title!", Color.green, null, null, null).build()).complete();
                } else {
                    //Had problem finding ID
                    currchat.sendMessage(createEmbed.methode("**ERROR**", "Could not find the Title you were looking for.", Color.red, null, null, null).build()).complete();
                }
            } else {
                //Error no correct number
                currchat.sendMessage(createEmbed.methode("**ERROR**", "Wrong Syntax! The **item ID** you typed is **wrong**, check the item ID in your inventory!", Color.red, null, null, null).build()).complete();
